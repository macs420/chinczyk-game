...
//Klasa do rysowania planszy
public class Layout {
    int x,y,width,height; //współrzędne x,y oraz szerokość i wysokość planszy
...
//Konstruktor klasy Layout
public void draw(Graphics2D g){
    g.setColor(Color.WHITE); //Kolor tła planszy
    g.fillRect(x,y,15*width,15*height); //Rysowanie tła planszy
    for(int i=0;i<6;i++) { //Rysowanie wewnętrznych kwadratów
    ...
    for(int i=1;i<6;i++) { //Rysowanie wewnetrznych pudełek 
        g.setColor(Color.RED);
    ...
    for(int i=0;i<2;i++) { //Rysowanie zewnętrznych puddełek 
        for(int j=0;j<2;j++) {
            g.setColor(Color.RED);
    ...
    int npoints = 3;
    g.fillPolygon(xpoints0, ypoints0, npoints); //Trójkąt na środku
    g.setColor(Color.YELLOW); 
    ...
    for(int i=0;i<3;i++) { //Rysowanie zewnętrznych pudełek
        for(int j=0;j<6;j++) {
    ...
    g.drawOval(x+5+(6*width),y+5+(2*height),width-10,height-10); //Rysowanie kółek na środku
    g.drawOval(x+5+(12*width),y+5+(6*height),width-10,height-10); //Rysowanie kółek na środku
    ...
    g.drawString("CZERWONY", 90, 35); //Tekst nad planszą 
    g.drawString("ZIELONY", 370, 35);
    g.drawString("NIEBIESKI", 90, 540);
    g.drawString("ŻÓŁTY", 370, 540);
    g.setFont(new Font("WEST JAVA", Font.BOLD, 20));
    g.drawString("NACIŚNIJ ENTER ŻEBY RZUCIĆ KOSTKĄ", 535,150);
    g.drawString("NACIŚNIJ NA PIONEK ŻEBY WYKONAĆ RUCH", 535,200);
} 
}
